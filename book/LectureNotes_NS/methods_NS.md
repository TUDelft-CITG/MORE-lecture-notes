# Statistical Modelling unders the Non Stationary assumptions

We will now explore two approaches to statistical modelling when time series present trends  or patterns. 
...
In the detrending approach we separates a time series $X_t$ into **deterministic** and **stochastic** components. In other words, the time series $X_t$ can be expressed as the sum of a deterministic component $ D(t) $, capturing the systematic effects such as trends or seasonality, and a stochastic component $ Y_t $:
...
For the specific case of a linear trend in the data, which could be tested for significance using the MK trend test, the series can be written as:
$ X_t = \beta_0 + \beta_1 t + Y_t$

where $beta_0$ and $\beta_1$ form the **deterministic component**, representing a fixed intercept and a linear trend over time, and $Y_t$ is the **stochastic component**. In this framework, one can estimate $beta_0$ and $\beta_1$ (e.g., using least squares), subtract the trend from the observations, and then develop statistical models exclusively for \( Y_t \). This approach is particularly useful when the focus is on understanding variability or extremes in the stochastic part without being influenced by the underlying trend.

Similarly, data with a strong seasonal signal can be investigated. For example monthly temperature. In this case, $D(t)$ can be a **sinusoidal function**.

# The integrated approach 
In the integrated approach we treat the time series $X_t$, including its deterministic component, as stochastic and we fit a statistical distribution to it with time dependent parameter(s). This approach implies that the distribution modelling $X_t$ would be of the same family, however, the parameters changes in accordance with the deterministic component $D(t)$ identified in the exploratory phase. Such construction can be generalized to follow not a temporal change but a change due to a related physically based covariates, e.g., changes in temperature due to the level of $CO_2$ in the atmosphere. 
...
<div style="background-color:#2b2b2b; border-left: 4px solid #888; padding: 12px; border-radius: 8px; color:#ddd;">
 <strong>Reference Literature.</strong> 
...
**Block Maxima and the Generalized Extreme Value Distribution**.  The **block maxima** method is a common approach in extreme value theory to select extreme events, where a time series is divided into blocks (e.g., yearly or monthly), and only the maximum value from each block is selected. It is possible to show that, the vector of maximum values per block concerge to a distribution of the **Generalized Extreme Value (GEV)** family distribution, which is given by
